...
class BinarySearchTree:
    ...
    def items_in_range(self, start: Any, end: Any) -> List:
        ...
        # If start is less than the root, search in the left subtree
        if self._left and start < self._root:
            left_items = self._left.items_in_range(start, end)
            if left_items is not None:
                items.extend(left_items)

        # If the current root is within the range, add it to the list
        if start <= self._root <= end:
            items.append(self._root)

        # If end is greater than the root, search in the right subtree
        if self._right and self._root < end:
            right_items = self._right.items_in_range(start, end)
    ...
    def rotate_right(self) -> None:
        ...
        if self._left is None:
            return  # No left subtree to rotate

        new_root = self._left
        self._left = new_root._right
        new_root._right = self

        # Move current root's data to new root
        self._root, new_root._root = new_root._root, self._root

        # Update root reference
        self._left, self._right = new_root._left, new_root._right
    ...
    def rotate_left(self) -> None:
        ...
        if self._right is None:
            return  # No right subtree to rotate
